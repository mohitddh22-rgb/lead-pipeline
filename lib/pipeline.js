...
import { logCronRun } from "./sheets.js";
...
export async function dispatchRegions(trigger = "manual") {
  ...
  // Durable log so the daily cron is verifiable in the Sheet (CronLog tab).
  await logCronRun({
    ...
    note: results.map((r) => `${r.region}:${r.status}`).join(" "),
  });
  return results;
}
