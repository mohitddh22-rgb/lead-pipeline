 ...
 * [office=estate_agent] / [shop=estate_agent] inside region bounding boxes.
 *
 * Overpass rejects requests without a User-Agent (HTTP 406). We send one and
 * use the dominant office=estate_agent tag so live data flows. Falls back to a
 * curated SEED list only if Overpass is truly unreachable.
 *
 * Uses the native global fetch (Node 18+) — no external dependency.
 */
...
async function queryOverpass(region) {
  ...
  for (const url of OVERPASS_MIRRORS) {
    try {
      ...
      if (els.length > 0) {
        console.log(`[scraper] ${region}: ${els.length} live Overpass leads (${url})`);
  ...
  console.warn(`[scraper] Overpass unavailable for ${region}: ${lastErr}`);
  return [];
}
