import { discoverRealEstateAgenciesOutscraper } from './outscraper.js';
import { enrichLeadsWithHunter } from './hunter.js';
...
export async function scrapeLeads(region, sources) {
  // 1. Primary: Outscraper (Google Maps) — free tier 500 places/mo, includes emails
  if (process.env.OUTSCRAPER_API_KEY) {
    try {
      const leads = await discoverRealEstateAgenciesOutscraper(region, {
        maxResults: 50,
      });
      if (leads.length) {
        console.log(`[scraper] Outscraper returned ${leads.length} leads for ${region}`);
        return leads;
      }
    } catch (e) {
      console.warn('[scraper] Outscraper discovery failed, falling back:', e.message);
    }
  }

  // 2. Secondary: Apollo — free org search, credit-gated enrich
  if (process.env.APOLLO_API_KEY) {
    try {
      ...
      if (leads.length) {
        console.log(`[scraper] Apollo returned ${leads.length} leads for ${region}`);
        return leads;
      }
    } catch (e) {
  ...
  // 3. Fallback: Explicit sources provided (Browserless)
  ...
  // 4. Fallback: Browserless discovery (requires BROWSERLESS_API_KEY)
