...
function buildOverpassQuery(region) {
  ...
  const [x1, y1, x2, y2] = c;
  return `[out:json][timeout:25];(node["shop"="estate_agent"](${x1},${y1},${x2},${y2});way["shop"="estate_agent"](${x1},${y1},${x2},${y2}););out center;`;
}
...
async function queryOverpass(region) {
  ...
  try {
    ...
    const els = (data.elements || []).filter((e) => e.tags && e.tags.shop);
